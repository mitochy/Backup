...
die "usage: R_biomart_gethumanhomolog.pl base_geneID_list.ssv <type (cdna, peptide, gene_flank, 3utr, 5utr, genomic, transcript_exon_intron)> <upstream need to get in bp> <R1 = get ID, R2 = get sequence>\n" unless @ARGV;
my ($input, $type, $upstream, $opt) = @ARGV;
my @datasets = mitochy::get_biomart_dataset_metazoa();
...
#Load human mart
$biomart_getid .= "celegans <-useMart(\"metazoa_mart_14\", dataset = \"celegans_eg_gene\")\n";

#Load requested human gene ID
$biomart_getid .= "celegans.geneid <- c($geneid)\n\n";
$biomart_getid =~ s/, \)/\)/i;
...
foreach my $datasets (@datasets) {
	...
	my ($org) = $datasets =~ /^(\w+)_eg_gene$/i;
	die "$datasets\norganism not defined (doesn't match org_eg_gene?)\n" unless defined($org);
	
	#Get Mart of each datasets
	$biomart_getid .= "$org.mart <- useMart(\"metazoa_mart_14\", dataset = \"$datasets\")\n";

	#Get ID for each dataset homolog to human
	$biomart_getid .= "$org.ID <- getLDS(attributes=c(\"ensembl_gene_id\"), filters = \"ensembl_gene_id\", values = celegans.geneid, mart = celegans, attributesL = c(\"ensembl_gene_id\"), martL = $org.mart)\n";
...
foreach my $org (sort keys %homolog) {
	...
	#Get Mart of each datasets
	$biomart_org_getseq .= "$org.mart <- useMart(\"metazoa_mart_14\", dataset = \"$org\_eg_gene\")\n";
