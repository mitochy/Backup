...
die "usage: R_biomart_TSSvalidator.pl fasta mode\nMode:\n<no mode> = take only those with >100 gene that has 5' UTR\nforce = force take everything\ntake = take good only\n" unless @ARGV;
...
foreach my $id (sort keys %fasta ) {
	...
	foreach my $protein2 (@{$protein{$id}{'seq'}}) {
		next if defined($mode) and $mode =~ /force/;
		$count++ if ($protein2 =~ /^$protein/i);
		$proteincheck = 1 if ($protein2 =~ /^$protein/i);
		last if ($protein2 =~ /^$protein/i);
	}
...
if ((not defined($mode) and (keys %good) > 100) or (defined($mode) and $mode =~ /force/)) {
	print "Printing...\n";
